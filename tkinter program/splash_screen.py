...
#     q= Tk()
#     q.geometry("427x250")
#     label1 = Label(w,text = "Add here",fg="dark grey",bg=None,font = ("Calibri",15))
...
unique1 = Tk()
unique1.title('Trinity Healthcare DB System')
unique1.iconbitmap('D:\\Programming folder\\Python programs\\tkinter program\\game.ico')
...
unique1.geometry("800x600")

my_menu = Menu(unique1)
unique1.config(menu=my_menu)

# click command
def our_command():
	my_label = Label(unique1, text="Clicked on a menu.").pack()
...
file_menu.add_command(label="Exit", command=unique1.quit)
...
button1_quit=Button(unique1, text="Exit Database", font=("Helvetica", 9),
command=unique1.quit)
button1_quit.pack()
...
class Hospital:
    ...
    def __init__(self,root):
        def upd():
            if nametext.get() =="":
                messagebox.showerror("Error","Enter all the required fields.")
            else:
                db = MySQLdb.connect(host = "localhost", user = "root", passwd = "", database = "hospital")
                mycursor = db.cursor()
                mycursor.execute("insert into patient_credentials values((%s,%s,%s,%s,%s))",
                ...
                db.commit()
                db.close()
                messagebox.showinfo("Success","The task has been completed successfully.")
        self.root=root 
        ...
        Symptoms_seen_text=Entry(DataFrameLeft,font=("arial",11),textvariable=self.symptoms,width=40)
        ...
        combo_department=ttk.Combobox(DataFrameLeft,state="readonly",font=("times new roman",13,"bold"),textvariable=self.department,width=30)
        ...
        Medical_History_text=Entry(DataFrameLeft,font=("arial",11),textvariable= self.history,width=36)
        ...
        Allergic_text=Entry(DataFrameLeft,font=("arial",11),textvariable=self.allergic,width=36)
        ...
        btnUpdate = Button(ButtonFrame, text = "Update",bg="blue",fg ="white", width =23,font = ("Times New Roman",12,"bold"),command = upd)
...
class Hospital:
    def __init__(self,root):
        def upd(self):
         if(self.Patient_Name.get()=="" or self.Address.get()==""or self.DOB.get() =="" or self.ID.get()=="" or self.Admission.get() =="" or self.Symptom.get()==""or self.Depart.get()=="" or self.Doctor.get()=="" or self.Medical_History.get()=="" or self.Diagnosis.get()=="" or self.Allergens.get()==""or self.Followup=):
            messagebox.showinfo("Error","All entries are required")
         else:
            conn=MySQLdb.connect(host="localhost",username="root",password="",database="hospital")
            my_cursor=conn.cursor()
            my_cursor.execute("insert into hospital values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",(self.Patient_Name.get(),self.Address.get(),self.DOB.get(),self.ID.get(),self.Admission.get()))
            conn.commit()
            conn.close()
            messagebox.showinfo("Record has been inserted")
        self.root=root
        self.root.title("HOSPITAL MANAGEMENT SYSTEM")
        self.root.geometry("1540x800+0+0")
        ...
        Diagnosis=Label(DataFrameLeft,text="Initial Diagnos`is:",font=("times new roman",14,"bold"),padx=3,pady=12)
        Diagnosis.grid(row=3,column=2)
        Diagnosis_text=Entry(DataFrameLeft,font=("arial",11),textvariable=self.Diagnosis,width=36)
        Diagnosis_text.grid(row=3,column=3)
        ...
        Prescription=Text(DataFrameRight,font=("arial",11),width=57,height=17,padx=2,pady=2)
        Prescription.grid(row=0,column=0)

        ######################################### BUTTONS MAKING #################################

        btnPrescription = Button(ButtonFrame,text = "Prescription",bg="blue",fg ="white", width =25,font = ("Times New Roman",12,"bold" ),padx=8)
        btnPrescription.grid(row=0,column = 1)

        btnPrescriptionData = Button(ButtonFrame, text = "Prescription Data", bg="blue",fg ="white", width =25,font = ("Times New Roman",12,"bold"),command = upd,padx=8 )
        btnPrescriptionData.grid(row=0,column = 2)

        btnUpdate = Button(ButtonFrame, text = "Update",bg="blue",fg ="white", width =25,font = ("Times New Roman",12,"bold" ),padx=8  )
        btnUpdate.grid(row = 0, column = 3)

        btnDelete = Button(ButtonFrame, text = "Delete",bg="blue",fg ="white", width =25,font = ("Times New Roman",12,"bold" ),padx=8 )
        btnDelete.grid(row = 0, column = 4)
        
        btnReset = Button(ButtonFrame, text = "Reset",bg="blue",fg ="white", width =25,font = ("Times New Roman",12,"bold" ),padx=8  )
        btnReset.grid(row = 0,column = 5 )

        btnExit = Button(ButtonFrame, text = "Exit",bg="blue",fg ="white", width =25,font = ("Times New Roman",12,"bold" ),padx=8 )
        btnExit.grid(row = 0,column = 6)
...
#,self.Symptom.get(),self.Depart.get(),self.Doctor.get(),self.Medical_History.get(),self.Diagnosis.get(),self.Allergens.get(),self.Followup.get()'''
